...
def simulate_round_robin(processes, quantum):
    ...
    while queue:
        ...
        print(
            f"[Time {current_time:02d}] "
            f"Process {pid} runs for {time_to_run} units."
        )
    ...
    avg_turnaround = (
        sum(turnaround_times.values())
        / len(processes)
    )

    print(
        f">> Round Robin Average Waiting Time: "
        f"{avg_wait:.2f}"
    )

    print(
        f">> Round Robin Average Turnaround Time: "
        f"{avg_turnaround:.2f}"
    )
